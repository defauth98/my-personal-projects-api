...
export class ProjectsController {
  ...
  async create(
    ...
    if (projectsAlreadyExits) {
      throw new BadRequestException('this project already exists');
    }
      ...
      if (process.env.NODE_ENV !== 'develop') {
        if (!files.gif || !files.thumbnail) {
          throw new BadRequestException(
            'You must provide the thumbnail and gif',
          );
        }
  ...
  async updateProject(
    ...
    if (!projectsAlreadyExits) {
      throw new BadRequestException('this project does not exit');
    }
